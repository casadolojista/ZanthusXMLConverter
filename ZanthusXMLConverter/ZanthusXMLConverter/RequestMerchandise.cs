...
namespace IntegracaoRequester {
    class RequestMerchandise {
        ...
        public static async void SearchForMerchandises(string requestEndpoint, string requestContentPath) {
            // List to set the found merchandises
            List<Merchandise> merchandises = new List<Merchandise>();

            // Merchandise's properties to search for
            ...
            using (var requestClient = new HttpClient()) {
                ...
                XDocument requestXDoc = XDocument.Load(requestContentPath);
                ...
                string fileBodyName = "SEARCH";
                string responseFileName = "merc_search_response.xml";
                string responseFileSaveLocation = @"C:\Users\Daniel\source\repos\XMLConverter\XMLConverter\Responses\";
                //responseXDocBody.Save(responseFileSaveLocation + "merc_search_response_proto.xml");
                FileWriter.WriteFromList(merchandises, searchedProperties, fileBodyName, responseFileName, responseFileSaveLocation);

                // Write objects list on console
                //foreach (Merchandise merc in merchandises) {
                //    Console.WriteLine("----------------------------------------");
                //    Console.WriteLine("Cod. Loja: " + merc.StoreID);
                //    Console.WriteLine("Cod. Mercadoria: " + merc.MercID);
                //    Console.WriteLine("Descrição: " + merc.Description);
                //    Console.WriteLine("Unidade de Venda: " + merc.SaleUnit);
                //}
